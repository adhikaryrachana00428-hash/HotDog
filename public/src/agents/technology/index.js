...
export async function runTechnologyAgent(context) {
    ...
    if (scraped.dependencyFiles['package.json']) {
        try {
            const manifest = JSON.parse(scraped.dependencyFiles['package.json']);
            const deps = { ...(manifest.dependencies || {}), ...(manifest.devDependencies || {}) };
            for (const dep of Object.keys(deps)) {
                const depKey = dep.split('/').pop()?.replace('@', '') || dep;
                if (TECHNOLOGY_REGISTRY[depKey])
                    addTech(depKey, `Detected in package.json as "${dep}".`);
