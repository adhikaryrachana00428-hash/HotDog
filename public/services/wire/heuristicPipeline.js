...
export async function runHeuristicPipeline(scraped, onProgress) {
    const emit = async (stage, status, pct) => {
...
function detectTechnologies(scraped) {
    ...
    const addTech = (key, insight) => {
        const base = DEPENDENCY_REGISTRY[key];
        if (!base || detected.has(key))
            return;
        detected.set(key, {
            id: key,
            name: base.name,
    ...
    if (scraped.dependencyFiles['package.json']) {
        try {
            const pkg = JSON.parse(scraped.dependencyFiles['package.json']);
            ...
            for (const dep of Object.keys(deps)) {
                ...
                if (dep.includes('next'))
                    addTech('next', 'Primary application framework listed in package.json.');
                if (dep.includes('prisma'))
                    addTech('prisma', 'Database ORM dependency in package.json.');
                if (dep.includes('react') && !dep.includes('react-'))
                    addTech('react', 'UI library dependency in package.json.');
                if (dep.includes('tailwind'))
                    addTech('tailwindcss', 'Styling framework in package.json.');
                if (dep.includes('clerk'))
                    addTech('clerk', 'Authentication provider in package.json.');
                if (dep.includes('redis') || dep.includes('ioredis'))
                    addTech('redis', 'Caching layer dependency in package.json.');
                if (dep.includes('pg') || dep.includes('postgres'))
                    addTech('postgres', 'PostgreSQL client dependency in package.json.');
            }
        }
        catch { /* invalid JSON */ }
    }
    ...
    if (scraped.primaryLanguage && !detected.size) {
        ...
        if (langKey.includes('typescript'))
            addTech('typescript', `Primary language: ${scraped.primaryLanguage}.`);
        if (langKey.includes('javascript'))
            addTech('react', `Primary language: ${scraped.primaryLanguage}.`);
        if (langKey.includes('python'))
            addTech('django', `Primary language: ${scraped.primaryLanguage}.`);
    }
...
function enrichTechnologies(technologies, scraped) {
    for (const tech of technologies) {
        if (!tech.whatItIs)
            tech.whatItIs = tech.role;
        if (!tech.whyProjectUses)
            tech.whyProjectUses = tech.insight;
    }
...
function buildArchitecture(scraped, technologies) {
...
function buildRelationships(technologies, blueprint) {
    const rels = [];
...
function buildAdvisor(scraped, technologies) {
...
function buildHiddenTools(technologies) {
    const tools = [];
...
function buildRecommendations(technologies) {
    const recs = [];
    if (technologies.some(t => t.key === 'prisma')) {
...
function buildRoadmap(technologies) {
...
function computeMetrics(scraped, technologies) {
    let pkgDepCount = 0;
    if (scraped.dependencyFiles['package.json']) {
        try {
            const pkg = JSON.parse(scraped.dependencyFiles['package.json']);
...
function buildConfidence(scraped, technologies) {
    return {
...
function assembleReport(scraped, parts) {
...
function buildDecisions(technologies) {
...
//# sourceMappingURL=heuristicPipeline.js.map