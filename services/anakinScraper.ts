...
export async function scrapeEngineeringResource(request: ScrapeRequest): Promise<ScrapeResult> {
...
function validateScrapeUrl(input: string): { valid: true; normalized: string } | { valid: false; error: string } {
...
async function fetchFromAnakin(url: string): Promise<AnakinJobResult> {
  ...
  try {
    ...
    const submit = (await submitResponse.json()) as AnakinJobResponse;
...
async function pollAnakinJob(jobId: string): Promise<AnakinJobResult> {
  const controller = new AbortController();
  ...
  try {
    ...
    return (await response.json()) as AnakinJobResult;
  } finally {
    clearTimeout(timeout);
  }
}

function normalizeScrapeResult(raw: AnakinJobResult, url: string, sourceType: ScrapedResource['source']): ScrapedResource {
...
function extractTitle(markdown: string, html: string, text: string, fallbackUrl: string): string {
  const fromMarkdown = markdown.match(/^#\s+(.+)$/m);
  if (fromMarkdown?.[1]) return fromMarkdown[1].trim();
  const fromHtml = html.match(/<title>([^<]+)<\/title>/i);
  if (fromHtml?.[1]) return fromHtml[1].trim();
  const fromText = text.match(/^.{1,120}/m);
  if (fromText?.[0]) return fromText[0].trim();
  return fallbackUrl;
}

function extractDependencies(markdown: string, html: string, text: string): string[] {
...
function extractFiles(markdown: string, html: string, text: string): string[] {
  const content = [markdown, html, text].join('\n');
...
function extractTechnologies(markdown: string, html: string, text: string): string[] {
  const content = [markdown, html, text].join('\n').toLowerCase();
  ...
  for (const candidate of candidates) {
    if (content.includes(candidate)) detected.add(candidate);
  }
  return [...detected];
}

function inferSourceType(url: string): ScrapedResource['source'] {
  const lower = url.toLowerCase();
  if (lower.includes('github.com')) return 'github';
  if (lower.includes('blog') || lower.includes('medium') || lower.includes('dev.to')) return 'blog';
  if (lower.includes('docs') || lower.includes('documentation')) return 'documentation';
  return 'resource';
}

export async function scrapeRepository(resourceUrl: string): Promise<ScrapeResult> {
  return scrapeEngineeringResource({ url: resourceUrl, sourceType: 'github' });
}

export async function scrapeDocumentation(resourceUrl: string): Promise<ScrapeResult> {
  return scrapeEngineeringResource({ url: resourceUrl, sourceType: 'documentation' });
}

export async function scrapeBlog(resourceUrl: string): Promise<ScrapeResult> {
