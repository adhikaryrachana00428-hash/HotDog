...
import { pollUntilComplete, type JobStatus } from './poll.js';
import { withRetry } from './retry.js';
import { runModularWirePipeline, type WireExecutionOptions } from '../src/agents/orchestrator.js';
...
const AGENT_STAGES: Array<{ stage: AnalysisProgress['stage']; name: string; pct: number }> = [
    ...
    try {
      ...
      if (!USE_HEURISTIC_FALLBACK) {
        throw new WireOrchestrationError(
          'Wire orchestration failed and heuristic fallback is disabled.',
          'WIRE_PIPELINE_FAILED',
          error
        );
      }
...
async function runLiveWirePipeline(
  scraped: NormalizedScrapedContent,
  onProgress: WireProgressCallback,
  options: WireExecutionOptions = {}
): Promise<FinalReport> {
  ...
  if (!validated.success) {
    ...
    if (!USE_HEURISTIC_FALLBACK) {
      throw new WireOrchestrationError(
        'Wire returned data that does not match EngineeringReport schema.',
        'WIRE_SCHEMA_INVALID',
        validated.error
      );
    }
    ...
    return validateFinalReport(mergeReports(heuristic, report as Partial<FinalReport>));
  }
  ...
  return validateFinalReport(validated.data as unknown as FinalReport);
}

/** Execute a pre-built Wire action with scraped content as parameters. */
async function executeWireTask(
  scraped: NormalizedScrapedContent,
  options: WireExecutionOptions = {}
  ...
  return (result.result || result.data || result) as Record<string, unknown>;
}

/** Use Wire Agentic Search for multi-stage engineering analysis. */
async function executeAgenticSearch(
  scraped: NormalizedScrapedContent,
  options: WireExecutionOptions = {}
): Promise<Record<string, unknown>> {
  ...
  const submit = await withRetry(
    () => wireFetch<{ job_id?: string; jobId?: string; id?: string }>('/agentic-search', {
      method: 'POST',
  ...
  const jobId = submit.job_id || submit.jobId || submit.id;
  if (!jobId) throw new WireOrchestrationError('Agentic search did not return a job ID.', 'NO_JOB_ID');
...
function buildAgenticSearchPrompt(scraped: NormalizedScrapedContent): string {
...
function parseWireOutput(
  ...
  if (jsonMatch) {
    try {
      const parsed = JSON.parse(jsonMatch[0]) as FinalReport;
  ...
  throw new WireOrchestrationError(
    'Could not parse structured JSON from Wire response.',
    'PARSE_FAILED'
  );
}

function mergeReports(base: FinalReport, partial: Partial<FinalReport>): FinalReport {
  return {
    ...base,
    ...partial,
...
async function wireFetch<T>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new WireOrchestrationError(
        `Wire API error (${response.status}): ${body}`,
        'WIRE_API_ERROR'
      );
    }
