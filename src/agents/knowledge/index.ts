import type { AgentContext, KnowledgeAgentOutput } from '../types.js';
import type { Technology } from '../../../types/schema.js';

export async function runKnowledgeAgent(context: AgentContext): Promise<KnowledgeAgentOutput> {
  const { scraped } = context;
  const technologies = context.technologyDiscovery?.technologies || [];

  const enriched = technologies.map((tech: Technology) => ({
    ...tech,
